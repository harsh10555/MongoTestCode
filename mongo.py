# NOSQL : In same table or collection we can store any unstructured format data
import pymongo
...
d = {                                        # data are stored in dictionary form
    "name":"Harsh",
    "email" : "hvsrajput@gmail.com",
    "surname" : "Vardhan"
}
list_of_records = [                                        # many data are stored in dictionary form (JSON)
    {'companyName': 'iNeuron',
...
db1 = client['mongotest']   # creating database name db1
coll = db1['test']      # creating collection (table) name coll inside database
#coll.insert_one(d )    # inserting data d or document in collection coll
coll.insert_many(list_of_records)

coll_2 = db1['table2']     # creating another collection (table) name coll_2 inside database
...
coll_2.insert_one(data1)

record =coll.find()    # fetching all records of coll collection
for i in record:
    print(i)